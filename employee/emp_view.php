...
echo '<div class="row">';
echo '<div class="col">';
echo '<h3 class="text-success mt-3 mb-3 display-5">Employee Chat Window</h3>';
echo'<hr>';
...
<div class="container">
  ...
  if(isset($_GET['task_id'])){
        $m_id = $_GET['task_id'];
    if($m_id!=0){
      $run = mysqli_query($link,"SELECT  * from `reply` join `wesite` on `wesite`.`id` =`reply`.`reply_by`  where `m_id` = '$m_id'");

      while($row = mysqli_fetch_array($run)){
        if($row['reply']!= NULL){
          echo'<div class="alert  text-center" style="background-color: #d9d9d9;"> By&nbsp;:&nbsp;'.$row['name'].'&nbsp;>&nbsp;'.$row['reply'].'</div>';}else{echo'';}}

      }else{header("location:reply.php");}} 
      else{header("location:reply.php");}
