...
echo '<div class="row">';
echo '<div class="col">';
echo '<h3 class="text-info mt-3 mb-3 display-3">Employee Chat Window</h3>';
echo'<hr>';
...
if(!empty($_GET['task_id'])){
    ...
    if($count>0){
       ...
       echo'<div class="col-md-12 col-sm-12 col-lg-12 mt-4 pl-4 pr-4 pt-4 pb-4" style ="background-color:#d9d9d9; padding-left: 40px;" class="">
    	'.$row['mesaage'].' </div>';
       '</div>';
       echo'<div>';
    }
...
if(isset($_GET['task_id'])){
  ...
  $my = mysqli_query($link, "INSERT INTO `$In_query`(`reply`, `m_id`, `reply_by`) VALUES ('$reply','$task_id','$u_id')");
...
<div class="container">
  ...
  if(isset($_GET['task_id'])){
    ...
    if($m_id!=0){
       ...
       $query = "SELECT  * from `reply` where `m_id` = '$m_id' AND `reply_by` = '".$_SESSION['user_id']."'";
       $run = mysqli_query($link,$query);

      while($row = mysqli_fetch_array($run)){

         if($row['reply']!= NULL){echo'<div class="alert  text-center" style="background-color: #d9d9d9;">'.$row['reply'].'</div>';}else{echo'';}}

      }else{header("location:reply.php");}} else{header("location:reply.php");}
